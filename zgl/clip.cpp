...
void gl_transform_to_viewport(GLContext *c,GLVertex *v)
{
  ...
  {
      v->zp.r=(int)(((v->color.r*(ZB_POINT_RED_MAX - ZB_POINT_RED_MIN))+(ZB_POINT_RED_MIN)));
      v->zp.g=(int)(((v->color.g*(ZB_POINT_GREEN_MAX - ZB_POINT_GREEN_MIN))+(ZB_POINT_GREEN_MIN)));
      v->zp.b=(int)(((v->color.b*(ZB_POINT_BLUE_MAX - ZB_POINT_BLUE_MIN))+(ZB_POINT_BLUE_MIN)));
  } 
  else 
  {
  ...
  {
    v->zp.s=v->tex_coord.x;
    v->zp.t=v->tex_coord.y;	
  }
}
...
static GLfloat name(glm::vec4 *c,glm::vec4 *a,glm::vec4 *b) \
{\
...
GLfloat (*clip_proc[6])(glm::vec4 *,glm::vec4 *,glm::vec4 *)=  {
    clip_xmin,clip_xmax,
    clip_ymin,clip_ymax,
    clip_zmin,clip_zmax
};
			     ...
			     GLVertex *q,GLVertex *p0,GLVertex *p1,GLfloat t)
{
  ...
  {
	  /*
	q->tex_coord.x = p0->tex_coord.x * (1-t) + p1->tex_coord.x * (t);
	q->tex_coord.y = p0->tex_coord.y * (1-t) + p1->tex_coord.y * (t);
